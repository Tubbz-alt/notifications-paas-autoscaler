...
class SqsScaler(AwsBaseScaler):
    ...
    def _get_desired_instance_count(self):
        ...
        desired_instance_count_throughput = self._get_desired_instance_count_based_on_throughput_of_tasks_pulled_from_queues()
        desired_instance_count_queue_length = self._get_desired_instance_count_based_on_current_queue_length()

        # We calculate the throughput of tasks put onto queues to record metrics for our dashboards. We don't actually need to
        # do anything with the data as we don't use it to calculate the desired instance count
        _get_total_throughput_of_tasks_put_onto_queues()
    ...
    def _get_throughput_of_tasks_put_onto_queue(self, queue):
        ...
        self.gauge("{}.queue-tasks-added-throughput".format(queue_name), past_5_mins_of_throughput[-1])
        return highest_throughput
